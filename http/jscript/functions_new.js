...
function fetch_movie_or_episode(LibraryKey, TriggeringElement) {
	...
	$.ajax({
		...
		dataType: "xml",
        beforeSend: function(xhr){xhr.setRequestHeader('X-Plex-Token', Token);},
		cache: false,
		...
		success: function(data) {
			...
			$(data).find("Directory").each(function() {
				...
				if (targetFunction !== false) {
					$("#SectionMenu").append("<li><a class='customlink' onclick='function_loader(\"" + targetFunction + "\",[\"" + $(this).attr("key") + "\", this]);'>" + $(this).attr("title") + "</a></li>");
					section = {
						key: $(this).attr("key"),
            ...
            $("#SectionMenu").append('<li><a class="customlink" onclick="function_loader(\'fetch_sections\');">Refresh Sections</a></li>');
			$('#LoadingModal').modal('hide');
		},
		error: function(data, status, statusText, responsText) {
			log_to_console(statusText);
...
function function_loader(function_name,function_args) {
    if($('#LoadingModal').hasClass('in') === false) {
        $('#LoadingModal').modal({keyboard: false, backdrop:'static', show:true});  
    }
	// First of all, update the settings.  headers: {'X-Plex-Token': Token},
	$.ajax({
		url: baseurl + utility + '?Func=PathExists&Path=.\&Secret='+Secret,
		cache: false,
		dataType: "text",
		global: false,
        beforeSend: function(xhr){xhr.setRequestHeader('X-Plex-Token', Token);},
...
function output_content(show_page) {
	$("#ContentBody").html("");
    var section = get_section_info(selected_section);
    $('#ContentHeader').html(section.title);
	...
	for (i = start_value; i < end_value; i++) {
		...
		if(item.hide === false) {
          ...
          <div class="panel-body" id="ContentBody">
                ...
                */
				newEntry = ['<div class="panel panel-default">']
                newEntry.push('<div class="panel-heading"><h4 class="panel-title">' + AppendToTitle + item.title + '</h4></div>');
				newEntry.push('<div class="panel-body"><table class="table table-bordered">');
                if (item.subtitles.length == 0) {
                    newEntry.push('<tr><td>No subtitles found.</td></tr>');   
                }
				for (x = 0; x < item.subtitles.length; x++) {
					...
					if (subtitle.hide === false) {
						...
						//newEntry.push("<div class='VideoSubtitle "+addClass+"'>");
						newEntry.push("<tr><td class='small mainText'>" + checkbox + "</td><td class='small mainText'>" + subtitle.codec + "</td><td class='small mainText'>" + language + "</td><td class='mainText'>" + subtitle.title + "</td><td class='small mainText'>" + view + "</td></tr>");
						if (addClass !== false) {
							newEntry.push("<tr><td class='mainText'>Message: " + active + exists + "</td></tr>");
						}
				...
				newEntry.push('<div class="panel-footer"><button class="btn btn-default btn-xs" onclick=\'subtitle_select_all("subtitle-'+item.id+'", true)\'>Select All</button> <button class="btn btn-default btn-xs" onclick=\'subtitle_select_all("subtitle-'+item.id+'", false)\'>Clear Selection</button> <button class="btn btn-default btn-xs" onclick=\'function_loader("subtitle_delete_confirm",["subtitle-'+item.id+'"]);\'>Delete Selected</button></div>');
			     newEntry.push('</div>');	
                } else if (item.type == "show") {
				newEntry.push("<div class='VideoBox'><div class='VideoHeadline'><span class='link' onclick='function_loader(\"fetch_show_or_season\",[\""+item.key+"\",false]);'>" + item.title + "</span></div></div>");
				} else if (item.type == "season") {
				newEntry.push("<div class='VideoBox'><div class='VideoHeadline'><span class='link' onclick='function_loader(\"fetch_movie_or_episode\",[\""+item.key+"\",false]);'>" + item.title + "</span></div></div>");
			}
