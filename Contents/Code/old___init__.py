...
def MainMenu(**kwargs):
	if not kwargs:
		...
		if setPMSPath():
			...
			oc.add(DirectoryObject(key=Callback(MainMenu), title='http://' + Prefs['PMS_Path'] + ':' + Prefs['WEB_Port'] + '/index.html'))
	...
	# Here comes the functions avail
		...
		if Func=='PathExists':
			...
			return GetXMLFileFromUrl(Secret, kwargs.get("Url"), kwargs.get("X-Plex-Token"))
		elif Func=='restart':
			return Restart()
...
def setPMSPath():
	...
	try:
#		Log.Debug('Token is: %s' %(MYTOKEN))
		#Let's see if we can add out subdirectory below this
		if MYTOKEN != '':
			tmpTest = XML.ElementFromURL('http://127.0.0.1:32400/?X-Plex-Token=' + MYTOKEN)
		else:
			tmpTest = XML.ElementFromURL('http://127.0.0.1:32400')
		return True		
	except:
...
def ValidatePrefs():
	global MYTOKEN
	MYTOKEN = getToken()
#	Log.Debug('Fetched token as : %s' %(MYTOKEN))
	setSecretGUID()
...
def GetXMLFileFromUrl(Secret, Url, XPlexToken):
	if PwdOK(Secret):
		import urllib2
#		Log.Debug('TOKEN IS : %s' %(XPlexToken))
		if MYTOKEN != None:
			Url = Url + '?X-Plex-Token=' + MYTOKEN
